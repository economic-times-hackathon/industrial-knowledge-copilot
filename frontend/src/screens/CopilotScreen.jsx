    ...
    <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`max-w-[85%] space-y-3 ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
        ...
        <div className={`rounded-xl px-4 py-3 text-sm ${
          ...
          {isUser
            ? <p>{msg.content}</p>
            : <div className="prose-industrial"><ReactMarkdown>{msg.content}</ReactMarkdown></div>
          }
        </div>
        ...
        {!isUser && msg.sources?.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {msg.sources.slice(0, 4).map(s => <SourceCard key={s.index} source={s} compact />)}
            {msg.sources.length > 4 && (
              <span className="text-[10px] text-gray-600 self-center">+{msg.sources.length - 4} more</span>
            )}
          </div>
        )}
      </div>
...
export default function CopilotScreen() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-88px)]">
      {/* Toolbar */}
      <div className="flex items-center gap-3 pb-3 border-b border-surface-600 mb-4">
        <Filter size={14} className="text-gray-500 shrink-0" />
        <select
          value={category}
          onChange={e => setCategory(e.target.value)}
          className="text-xs bg-surface-700 border border-surface-500 rounded-lg px-3 py-1.5 text-gray-300 focus:outline-none focus:border-accent-blue"
        >
        ...
        {messages.length === 0 && (
          <div className="space-y-4 py-4">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500">Ask anything about your industrial knowledge base.</p>
              <p className="text-xs text-gray-700 mt-1">102 documents · 13,779 indexed chunks · GPT-4o</p>
            </div>
      ...
      <div className="mt-4 pt-4 border-t border-surface-600">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-surface-700 border border-surface-500 rounded-xl px-4 py-2.5 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-accent-blue transition-colors"
          />
          <button
            onClick={() => send()}
        ...
        <p className="text-[10px] text-gray-700 mt-1.5 pl-1">Enter to send · Shift+Enter for newline</p>
      </div>
    </div>
  )
}
